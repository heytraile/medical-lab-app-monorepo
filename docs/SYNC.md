# Connection (sync) screen — what it is and how it works

This page explains the **Connection** menu in the app (`/sync`): what you are looking at, what the numbers mean, and how behaviour differs when you are developing on your own machine versus running the lab for real in the cloud.

---

## Why Connection exists at all
...
Important things (new patients, new accessions, test results, “submit for release”, recalls) are **saved on the lab computer first**, then **copied up to the cloud** when a connection is available. The Connection screen is your window into that copy process.

You do not need to open Connection for every day-to-day task. Bench, Accession, and Release still work as normal. Connection is mainly for **checking that nothing is stuck** and for **pushing waiting items up immediately** when the authorizer’s release queue looks empty but the bench says something was already submitted.

---

## What you see on the Connection screen

The page title is **Connection**. Below that you get counters and a **Send now** button.

### The counters

| UI label | Plain meaning |
| --- | --- |
| **Waiting to send** | Messages queued **or currently sending** (`pending` + `syncing` on the lab PC). Includes work in flight so the number does not flash to zero mid-send. |
| **Could not send** | Something went wrong when sending. The lab computer will try again automatically; you can also use **Send now**. |
| **Sent (recent sync messages)** | Items that were sent and the cloud confirmed receipt. These stay in the history on the lab computer; they are not “unsent”. |

The page refreshes every few seconds so you can watch numbers move without reloading.

### Normal flicker (0 ↔ 1)

When the lab is active (results arriving, manual entry, submit for release), you may see **Waiting to send** alternate between **0** and **1** for a moment. That is **normal** if **Could not send** stays at **0** — one message was queued and the automatic send (about every 15 seconds) cleared it. Worry if **Could not send** rises or **Waiting to send** stays stuck above zero for several minutes.

### Send now

**Send now** tells the lab computer: *send everything that is waiting right now*.

Use it when:

- A bench tech clicked **Submit for release** but the **Release queue** is still empty.
- **Waiting to send** or **Could not send** counts are above zero and you want to nudge the copy process instead of waiting.

Many actions elsewhere in the app (submit, recall, return to bench, manual entry) already try to send automatically. **Send now** is the manual “send it now” button when you want to be sure.
...
### How Connection behaves locally

1. Results and accessions land on the lab computer first (Bench updates immediately).
2. Messages queue as **pending** (shown under **Waiting to send**).
3. Every ~15 seconds the lab computer tries to send waiting messages to the local cloud service, **or** you click **Send now**.
4. If the send succeeds, items move to **Sent (recent sync messages)** and the Release queue / reports can see the updated cloud copy.
5. If the cloud service or local database is not running, **Waiting to send** stays above zero until it comes back.
...
- **Send now** on Connection is a normal part of local testing when the release queue does not update immediately after submit.
...
| If internet drops | Lab bench keeps working; sends queue as **Waiting to send** | Same — bench keeps working; sends wait until internet returns |
...
### How Connection should behave in production

1. **Automatic sending** — The lab computer tries to send waiting messages on a schedule (about every 15 seconds). Staff should rarely need **Send now** if the network is healthy.
2. **Waiting to send during outages** — If the internet or cloud is down, the count rises. Nothing is lost on the lab computer. When connectivity returns, sends resume and counters should drop.
3. **Could not send needs attention** — A steady failed count means something is wrong (wrong credentials, cloud unreachable, bad data). Check network and contact support; **Send now** alone may not fix a persistent failure.
4. **Sent is historical** — A large sent count is normal over time. It means “successfully copied in the past”, not “still waiting”.
5. **Authorizers depend on sync for submit** — Releasing results happens in the cloud. A tech’s **Submit for release** on Bench only reaches the authorizer after that message is delivered. That is why an empty Release queue with **Waiting to send > 0** on Connection means “wait or Send now”, not “nothing was submitted”.

Production does **not** require staff to open Connection every day. It is a **health and troubleshooting** screen: confirm the pipe between lab and cloud is moving.
...
| --- | --- | --- |
| Release queue empty after submit | Message not in cloud yet | Open **Connection** → check **Waiting to send** → **Send now** |
| **Waiting to send** stays high | Cloud unreachable or not running | Local: start Docker / cloud service. Production: check internet |
| **Could not send** > 0 | Last send attempt errored | **Send now** once; if it persists, check logs / support |
| **Waiting to send** flickers 0↔1, failed is 0 | Healthy churn while lab is active | No action needed |
| Sent count grows, waiting is 0 | Healthy — copies are getting through | No action needed |
